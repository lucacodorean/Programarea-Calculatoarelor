...
int min(int count,...) {
    va_list arguments;
    va_start(arguments, count);
    int result_min=MAX_ELEMENT;
    for(size_t i = 0; i<count; i++) {
        result_min = aux_min(result_min, va_arg(arguments, int));
    }
    va_end(arguments);
    return result_min;
}

int aux_max(int a, int b) {
    return (a>b ? a : b);
}
...
int max(int count,...) {
    ...
    int result_max = MIN_ELEMENT;
    for(size_t i = 0; i<count; i++) {
        result_max = aux_max(result_max, va_arg(arguments, int));
    }
    va_end(arguments);
    return result_max;
}
